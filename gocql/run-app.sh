...
if grep "FAIL:" "gocql-tests.log"; then
  ...
  python $WORKSPACE/integrations/utils/create_json.py --test_name "NA" --script_name "gocql-test" --result FAILED --file_path stack4json.log >> temp_report.json
  RESULT=1
else
  python $WORKSPACE/integrations/utils/create_json.py --test_name "NA" --script_name "gocql-test" --result PASSED >> temp_report.json
fi
